...
postdd(){
            ...
            [ "$type" = ext4 -o "$type" = xfs -o "$type" = btrfs ] && {
              ...
              [ $staticnetinfo != '' -a $ISDHCP != '1' ] && [ -f $therealospath/etc/network/interfaces ] && sed -i "s/iface eth0 inet dhcp/iface eth0 inet static\n  address $IP\n  netmask $MASK\n  gateway $GATE/g" $therealospath/etc/network/interfaces
              [ $ISDHCP = '1' ] && [ -f $therealospath/etc/network/interfaces ] && [ -z "$(sed -n '/iface eth0 inet dhcp/p' $therealospath/etc/network/interfaces)" ] && printf "auto eth0\niface eth0 inet dhcp\n" >> $therealospath/etc/network/interfaces             
...
exit 0\n" >> "$therealospath"/etc/init.d/rc.local
              ...
              [ "$CMDSTR" != '' -a "$CMDSTR_ORI" != '' ] && echo '#!/bin/bash' > $therealospath/etc/rc.local && printf "%b" "$CMDSTR_ORI" >> $therealospath/etc/rc.local && chmod +x $therealospath/etc/rc.local
              ( umount osmnt )
            }
            [ "$layout" = "dsm" ] && {
              mkdir thebootpart
            ...
            [ "$type" = ntfs ] && {
              ...
              [ "$CMDSTR" != '' -a "$CMDSTR_ORI" != '' ] && printf "%b" "$CMDSTR_ORI"| iconv -f 'UTF-8' -t 'GBK' >> /thebiggestlastospart/Windows/System32/GroupPolicy/Machine/Scripts/Startup/cloudinit.bat
              printf "%b" "@echo\0040off\r\nnet\0040session\0040\003e\006e\0075\006c\0040\0032\003e\0040\0026\0031\r\nif\0040\005f\0025errorlevel\0025\0040neq\0040\0030\0040\0028\r\n\r\n    powershell\0040-Command\0040\0022Start-Process\0040'\005e\0025~f0'\0040-Verb\0040RunAs\0022\r\n    exit\0040/b\r\n\0029\r\nreg\0040delete\0040\0022HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Run\0022\0040\0057v\0040setupChrome\0040\0057f\r\ndel\0040\0022C:\\Users\\Administrator\\Downloads\\setupChrome.bat\0022\r\nstart\0040\0022\0022\0020\0022C:\\Users\\Administrator\\Downloads\\ChromeSetup.exe\0022\r\n\0028goto\0029\0040\0032\003e\006e\0075\006c\0040\0026\0040del\0040\0022\005e\007e\0066\0030\0022\r\n" | iconv -f 'UTF-8' -t 'GBK' - >> /thebiggestlastospart/Windows/System32/GroupPolicy/Machine/Scripts/Startup/cloudinit.bat
              # [ -d ] before cd cp?
